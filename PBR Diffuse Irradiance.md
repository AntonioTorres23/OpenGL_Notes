...
This gives us an integral that only depends on $\Large{\omega_i}$ (assuming $\Large{p}$ is at the center of the environment map). With this knowledge, we can calculate or *pre-compute* a new cubemap taht stores in each sample direction 