...
Remember that the `xpos` and `ypos` are the horizontal mouse-movements (yaw) and the vertical mouse-movements  (`pitch`) whenever the mouse is moved. So when we bound our `mouse_callback` function to `glfwSetPosCursorCallback`, whenever the mouse makes a movement it calls the `mouse_callback`. Within the `mouse_callback` function parameter/arguments, we take in a pointer of our GLFW window we created previously in the main file as well as `xpos` and `ypos` which are the yaw and pitch $\theta$ (theta) angles we use to calculate the rotation and direction the position