...
So I think of it like this, you have your `cameraPos` vector and whenever in the while loop a w key is pressed that is going forward so it will take the scalar of the camera (in this case 0.5) and  multiply it by the `cameraFront` vector (the global vector being (0, 0, -1)). This gives us the vector (0,0,-0.5). Then we add this with the camera's current position vector, (which starts at (0, 0, 3)), so the equation would be $(0,0,3)$ + $(0,0,-0.05)$
