...
What is interesting to note here is the division of `viewDir.xy` by `viewDir.z`. As the `viewDir` vector is normalized, `viewDir.z` will be somewhere in the range between $0.0$ and $1.0$. When  `viewDir` is largely parallel to the surface, its z component is close to 0
